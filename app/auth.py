...
def create_access_token(user: User) -> str:
    ...
    payload = {"sub": str(user.id), "org": user.org_id, "role": user.role,
               "jti": uuid.uuid4().hex, "iat": iat,
               "exp": iat + int(lifetime.total_seconds()), "type": "access"}
...
def create_refresh_token(user: User) -> str:
    ...
    payload = {"sub": str(user.id), "org": user.org_id, "role": user.role,
               "jti": uuid.uuid4().hex, "iat": iat,
               "exp": iat + int(lifetime.total_seconds()), "type": "refresh"}
    return jwt.encode(payload, JWT_SECRET, algorithm=JWT_ALGORITHM)
