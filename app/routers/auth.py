...
from ..auth import (consume_refresh_token, create_access_token, create_refresh_token,
                    decode_token, get_token_payload, hash_password, revoke_access_token,
                    verify_password)
...
def register(payload: RegisterRequest, db: Session = Depends(get_db)):
    ...
    user = User(org_id=org.id, username=payload.username,
...
def login(payload: LoginRequest, db: Session = Depends(get_db)):
    org = db.query(Organization).filter(Organization.name == payload.org_name).first()
    user = None if org is None else db.query(User).filter(
        User.org_id == org.id, User.username == payload.username).first()
