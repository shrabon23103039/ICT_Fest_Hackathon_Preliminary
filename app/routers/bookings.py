...
# A process-wide critical section makes conflict/quota creation and cancellation atomic
# for the single-process Uvicorn deployment supplied by this challenge.
...
def create_booking(payload: BookingCreateRequest, db: Session = Depends(get_db),
    ...
    with _booking_lock:
        ...
        booking = Booking(room_id=room.id, user_id=user.id, start_time=start, end_time=end,
                          status="confirmed", reference_code=reference.next_reference_code(),
...
@router.get("/bookings")
def list_bookings(page: int = Query(1, ge=1), limit: int = Query(10, ge=1, le=100),
                  db: Session = Depends(get_db), user: User = Depends(get_current_user)):
    base = db.query(Booking).filter(Booking.user_id == user.id)
    total = base.count()
    items = base.order_by(Booking.start_time.asc(), Booking.id.asc()).offset(
...
def get_booking(booking_id: int, db: Session = Depends(get_db),
                user: User = Depends(get_current_user)):
    booking = db.query(Booking).join(Room, Booking.room_id == Room.id).filter(
        Booking.id == booking_id, Room.org_id == user.org_id).first()
    if booking is None or (user.role != "admin" and booking.user_id != user.id):
        raise AppError(404, "BOOKING_NOT_FOUND", "Booking not found")
    response = serialize_booking(booking)
    response["refunds"] = [{"amount_cents": r.amount_cents, "status": r.status,
                            "processed_at": iso_utc(r.processed_at)} for r in booking.refunds]
    return response
...
def cancel_booking(booking_id: int, db: Session = Depends(get_db),
    ...
    return {"id": booking.id, "status": "cancelled", "refund_percent": refund_percent,
