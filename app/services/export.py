...
EXPORT_HEADER = ["id", "reference_code", "room_id", "user_id", "start_time",
                 "end_time", "status", "price_cents"]
...
def generate_export(db: Session, org_id: int, user_id: int, room_id: int | None, include_all: bool) -> str:
    rows = _fetch_scoped(db, org_id, None if include_all else user_id, room_id)
    buffer = io.StringIO()
    writer = csv.writer(buffer)
    writer.writerow(EXPORT_HEADER)
    for b in rows:
        writer.writerow([b.id, b.reference_code, b.room_id, b.user_id, iso_utc(b.start_time),
                         iso_utc(b.end_time), b.status, b.price_cents])
